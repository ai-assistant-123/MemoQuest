...
import { ArrowLeft, Eye, EyeOff, CircleHelp } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 flex flex-col h-screen max-h-screen">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-start">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-cyan-400 game-font">探险模式</h2>
            </div>
          ...
          {/* 难度等级选择器 */}
          <div className="flex bg-gray-900 p-1 rounded-lg overflow-x-auto max-w-full">
            {[1, 2, 3].map((lvl) => (
              <button
                key={lvl}
                onClick={() => handleLevelChange(lvl)}
                className={`px-3 md:px-4 py-2 rounded-md font-bold text-sm transition-all whitespace-nowrap ${
                  level === lvl
      ...
      <div className="flex-grow overflow-hidden relative bg-gray-900 rounded-xl border-4 border-gray-700 shadow-inner flex flex-col">
        ...
        {/* 底部状态提示 */}
        <div className="bg-gray-800 p-2 text-center text-xs text-gray-500 font-mono border-t border-gray-700 flex justify-between px-4">
           <span>{showOriginal ? '阅读模式：请通读原文' : '点击方块显示文字 • 朗读尝试还原'}</span>
